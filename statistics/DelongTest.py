...
data = pd.read_excel(r'preds_internal_test.xlsx', sheet_name=0, header=None)
train_data = np.array(data)  # np.ndarray()
excel_list = train_data.tolist()  # list
excel_listV1 = list(map(list, zip(*excel_list)))
preds_S1 = excel_listV1[0]
preds_S2 = excel_listV1[1]
preds_S3 = excel_listV1[2]
preds_S4 = excel_listV1[3]
Result = excel_listV1[4]
# strategyX (random) vs. "good" model
DelongTest(preds_S3, preds_S4, Result)
